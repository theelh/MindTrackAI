    ...
    public function analyzeAudioEmotion(string $s3path)
{
    ...
    public function analyzeAudioEmotionApi(string $s3path)
{
    try {
        // Step 1: Convert audio to text (speech-to-text)
        $transcribed = $this->speechToTextApi($s3path);
...
public function speechToTextApi(string $audioUrl): ?string
{
    try {
        // 1️⃣ Get audio binary (local or remote)
        if (str_contains($audioUrl, url('/storage'))) {
            $relative = str_replace(url('/storage') . '/', '', $audioUrl);
            $audioBinary = Storage::disk('public')->get($relative);
        } else {
            $audioBinary = Http::timeout(60)->get($audioUrl)->body();
        }

        if (empty($audioBinary)) {
            Log::error('🎤 Empty audio binary');
            return null;
        }

        // 2️⃣ Send RAW audio binary (NO JSON)
        ...
        if ($response->failed()) {
            Log::error('🎤 Whisper failed: ' . $response->body());
            return null;
        }
